...
### 2026-09-26 御膳房项目二维码内嵌
- 按用户批注移除“掌上御膳房”顶部大幅小程序码，在项目简介正文下方展示 148px 网页访问二维码（窄屏 132px）与直接访问链接；不裁切二维码。
- 首页静态条目和 CMS 动态渲染均生效，不修改后台存储数据。该项目原 image 字段不再用作封面。
- 新增自托管 qrcode-generator 1.4.4 与 `js/project-access.js`，二维码从项目访问链接生成；后台更新项目链接后二维码同步变化，无外部二维码服务。当前目标为 https://yushanfang.gezhenghao.com/。
- 已核对页面无旧封面、二维码完整显示；用独立解码器核对生成内容与访问链接一致。
