...
(function() {
  ...
  window.loadArticle = async function(filename) {
    ...
    if (!filename || typeof filename !== 'string') {
      content.innerHTML = '<p class="error">文章标识无效</p>';
      return;
    }
    ...
    // 如果 content-loader.js 已经接管且命中后台缓存，优先走后台渲染
    if (window.__articlesCache && Array.isArray(window.__articlesCache)) {
      const cached = window.__articlesCache.find(a => a && a.slug === filename);
      if (cached && typeof window.__loadArticleFromCache === 'function') {
        try { return window.__loadArticleFromCache(filename); } catch (e) {}
      }
    }
